...
class Tour:
    ...
    def generate_tour(self):
        ...
        match = ((groupe1[i]["nom"], groupe2[i]["nom"]),
                (groupe1[i]["score"], groupe2[i]["score"]))
        matchs.append(match)
        ...
        while compteur < 9 and reponse_utilisateur == True:
            ...
            for i in range(0, 8):
                ...
                for j in range(i+1, 8):
                    ...
                    print(p1, p2)
                    match = ((playerslists_ordering[i]["nom"],
                    playerslists_ordering[j]["nom"]),
                    (playerslists_ordering[i]["score"],
                    playerslists_ordering[j]["score"]))
                    if self.check_match(p1, p2, matchs) == False:
                        if self.check_player(p1, p2, current_round) == False:
                            current_round.append(match)
                            matchs.append(match)
                            self.update_score(playerslists_ordering[i],
                            playerslists_ordering[j])
                            break
            print(matchs)
            compteur += 1
    ...
    def check_match(self, p1, p2, matchs):
        for item in matchs:
            if (p1 == item[0][0] and p2 == item[0][1]) or (
                            p1 == item[0][1] and p2 == item[0][0]):
                return True
        return False

    def check_player(self, p1, p2, match_round2):
        for item in match_round2:
            if (p1 == item[0][0] or p1 == item[0][1]) or (
                            p2 == item[0][0] or p2 == item[0][1]):
                return True
        return False
